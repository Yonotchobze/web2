...
router.get('/', function(req, res, next) {
  let minimumDuration = req?.query['minimum-duration'] ? parseInt(req.query['minimum-duration']) : undefined;
  if (minimumDuration && (typeof minimumDuration !== 'number' || minimumDuration < 0)) return res.sendStatus(400);
  if (!minimumDuration) return res.json(films);
  const filmsWithMinimumDuration = films.filter((film) => film.duration >= minimumDuration);
  return res.json(filmsWithMinimumDuration);
});
